...
from resume_structure import merge_with_inventory, render_resume_markdown
from schemas import (
    GeneratedDraft,
    ParsedResume,
    RefinedResume,
...
SYSTEM_EVALUATE = (
    ...
    "must always be a JSON array of exactly five strings. Return only valid JSON."
)
...
def process_resume(
    ...
    draft_json = merge_with_inventory(parsed, draft.resumeJson, draft.bulletRewrites)
    draft_markdown = render_resume_markdown(draft_json)

    evaluation_raw = generate_json_content(
        contents=build_evaluate_resume_prompt(
            draft_resume_markdown=draft_markdown,
    ...
    final_json = merge_with_inventory(parsed, refined.resumeJson, refined.bulletFeedback)
    final_markdown = render_resume_markdown(final_json)

    return merge_generated_resume(
        draft,
        evaluation,
        refined,
        draft_resume_markdown=draft_markdown,
        refined_resume_markdown=final_markdown,
        resume_json=final_json,
    )
